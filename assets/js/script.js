  ...
  fetch(nationalParkApi).then(function (response) {
    if (response.ok) {
      response.json().then(function (data) {
        ...
        L.marker([lat, long]).addTo(map).bindPopup(data.data[0].fullName).openPopup();
...
function createHikePage(parks, activities) {
  ...
  for (var i = 0; i < activities.data.length; i++) {
    ...
    if (
      activities.data[i].activities[0].name.toLowerCase() === 'hiking' ||
      'backpacking' ||
      'stargazing' ||
      'recreation' ||
      'walking'
    ...
    if (response.ok) {
      response.json().then(function (data) {
        ...
        for (let i = 0; i < fBoxEl.length; i++) {
          fBoxEl[i]
            .querySelector('img')
            .setAttribute('src', 'https://openweathermap.org/img/wn/' + data.daily[i].weather[0].icon + '.png');
...
let visitorPageInfo = function (pData) {
  ...
  cUsInfoEl.innerText =
    'Contact Us:\n' +
    'Phone: ' +
    pdata.contacts.phoneNumbers[0].phoneNumber +
    '\nEmail: ' +
    pdata.contacts.emailAddresses[0].emailAddress +
    '\n';
};
